...
// 값 타입 : 변수 안에 실제 데이터(값)이 있는 경우. 장점 : 값 복사 편함. 대입(=)만으로 값 복사 가능함.
...
// NS : Next Step의 약자 스티브잡스 아이폰이나 맥의 OS 기초를 만든 회사 이름.
...
print(myClass.age)
print(myClass2.age)
// 힌트 : myClass와 myClass2는 주소값이 동일함. 같은 메모리 공간을 가리키고 있음.